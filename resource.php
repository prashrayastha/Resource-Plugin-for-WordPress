...
// Add custom column for Item No
// Use the edit-{$post_type}_columns hook so it targets the 'resources' CPT list table in admin
add_filter('manage_edit-resources_columns', 'resource_add_item_no_column');
function resource_add_item_no_column($columns) {
    // Create new array and insert "Item No." before Title column
    $new_columns = [];
    $new_columns['cb'] = $columns['cb']; // keep checkbox first
    $new_columns['item_no'] = __('Item No.', 'resource-plugin');

    // Then merge remaining columns except original 'cb'
...
// Output for Item No. column
// Use the manage_{$post_type}_posts_custom_column hook for the 'resources' CPT
add_action('manage_resources_posts_custom_column', 'resource_render_item_no_column', 10, 2);
function resource_render_item_no_column($column, $post_id) {
    if ($column === 'item_no') {
        // Show the actual post ID for the resource.
        // Link to the Edit screen when available for convenience.
        $edit_link = get_edit_post_link( $post_id );

        if ( $edit_link ) {
            echo '<a href="' . esc_url( $edit_link ) . '">' . esc_html( $post_id ) . '</a>';
        } else {
